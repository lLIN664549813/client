            ...
            <block v-if="type !== 'WORD_IMAGE_CLICK'">
                ...
                <view class="verify-content">
                    <view class="verify-body">
                        ...
                        <view v-if="type === 'CONCAT'" id="verify-concat-bg" class="verify-concat-bg" :style="imgStyle">
                        </view>
                        <view v-else class="verify-slider" :style="imgStyle">
                            <image id="slider-img" :src="captchaData.slider_image" mode="heightFix"></image>
                        </view>
            ...
            <view v-else class="verify-content">
                ...
                <view class="verify-body">
                    ...
                    <view id="image-click-mask" class="image-click-mask" @click="recordClickItem">
                        <view v-for="(item, index) in captchaData.trackArr" :key="index" class="click-item"
                            :style="{ left: `${item.x - 15}px`, top: `${item.y - 15}px` }">{{ index + 1 }}</view>
                    </view>
                </view>
            </view>
...
<script>
import { getCaptcha, validateCaptcha } from "@/api/login";
export default {
    name: "verify-code",
    props: {
        type: {
    ...
    methods: {
        ...
        getVerifyData() {
            ...
            getCaptcha().then(res => {
                this.captchaData.id = res.id;
                this.captchaData.background_image = res.captcha.backgroundImage;
                this.captchaData.slider_image = res.captcha.templateImage;
        ...
        setVertifyData() {
            ...
            validateCaptcha(dataForm).then(res => {
                console.log("res", res);
                if (res.code == "200") {
                    if (res.data) {
                        this.isSuccess = true;
